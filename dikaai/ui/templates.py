...
:root {
  ...
  --shadow-border: 0 2px 0 0 var(--border-strong);
...
.topbar-logo{display:flex;align-items:center;gap:8px;font-size:17px;font-weight:800;color:var(--primary-light)}
...
.status-dot{display:flex;align-items:center;gap:6px;font-size:12px;color:var(--success);font-weight:600}
...
@media(min-width:768px){.topbar-nav{display:flex}.topbar{padding:10px 24px}}
"""
...
.panel-head svg{width:16px;height:16px;margin-right:6px;opacity:0.6}
...
.toggle-label svg{width:16px;height:16px;color:var(--primary-light)}
...
CHAT_PAGE_CSS = """
.chat-wrap{display:flex;flex:1;overflow:hidden;height:calc(100vh - 52px)}
.chat-sidebar{
  width:260px;background:var(--surface);border-right:2px solid var(--border);
  display:flex;flex-direction:column;overflow:hidden;
  transition:var(--transition);
}
.sidebar-title{padding:16px;font-size:12px;color:var(--text-3);text-transform:uppercase;letter-spacing:1px;font-weight:700;border-bottom:2px solid var(--border);display:flex;align-items:center;gap:8px}
.sidebar-title svg{width:14px;height:14px}
.mem-list{flex:1;overflow-y:auto;padding:10px}
.mem-item{
  padding:12px;border-radius:var(--radius-sm);margin-bottom:6px;
...
.mem-item .mem-label svg{width:12px;height:12px}
.mem-item .mem-val{color:var(--text);font-size:14px;font-weight:600}
...
.msg-ai{
  ...
  border:2px solid var(--border);border-bottom-left-radius:4px;
}
.msg-ai:hover{border-color:var(--primary-border)}
.msg-meta{font-size:11px;color:var(--text-3);margin-top:6px;display:flex;gap:10px;align-items:center}
...
.msg-bubble pre{background:var(--bg);padding:12px;border-radius:var(--radius-xs);overflow-x:auto;margin:8px 0;border:2px solid var(--border)}
.msg-bubble pre code{font-family:var(--mono);font-size:13px;color:var(--accent)}
.msg-bubble code{background:var(--bg);padding:2px 6px;border-radius:4px;font-family:var(--mono);font-size:13px}
.typing-ind{display:flex;gap:5px;padding:14px 18px;background:var(--surface-2);border:2px solid var(--border);border-radius:var(--radius);border-bottom-left-radius:4px;width:fit-content}
...
.chat-input-area{padding:12px 16px;background:var(--surface);border-top:2px solid var(--border)}
.chat-input-wrap{display:flex;gap:8px;max-width:860px;margin:0 auto}
.chat-input-wrap textarea{
  flex:1;background:var(--surface-2);border:2px solid var(--border);
...
.send-btn{
  background:var(--primary);color:#fff;border:2px solid var(--primary-dark);
  border-radius:var(--radius-sm);padding:0 20px;cursor:pointer;
  font-weight:700;font-size:14px;transition:var(--transition);
  display:flex;align-items:center;gap:6px;min-width:60px;justify-content:center;
}
...
.welcome-box h2{font-size:28px;font-weight:800;margin-bottom:6px;background:linear-gradient(135deg,var(--primary),var(--accent));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
@media(max-width:768px){
  .chat-sidebar{position:fixed;top:0;left:0;height:100%;z-index:50;width:280px;transform:translateX(-100%)}
...
@media(min-width:769px){.chat-sidebar{transform:none!important}}
"""

API_DOCS_CSS = """
.docs-body{padding:40px 20px;max-width:900px;margin:0 auto}
.docs-body h1{font-size:32px;font-weight:800;margin-bottom:6px;background:linear-gradient(135deg,var(--primary),var(--accent));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.docs-body .sub{color:var(--text-2);margin-bottom:28px;font-size:14px}
.docs-body h2{font-size:20px;font-weight:700;margin:32px 0 14px;color:var(--primary-light);border-bottom:2px solid var(--border);padding-bottom:8px}
.endpoint{
  background:var(--surface);border:2px solid var(--border);
  border-radius:var(--radius-sm);padding:16px;margin-bottom:10px;
  transition:var(--transition);
}
.endpoint:hover{border-color:var(--primary-border)}
.endpoint .method{display:inline-block;padding:3px 10px;border-radius:var(--radius-xs);font-size:12px;font-weight:800;margin-right:8px;font-family:var(--mono)}
...
.endpoint .desc{color:var(--text-2);font-size:13px;margin-top:8px}
.endpoint .auth{color:var(--warning);font-size:11px;margin-top:4px;display:flex;align-items:center;gap:4px}
.endpoint .auth svg{width:12px;height:12px}
pre{background:var(--surface);border:2px solid var(--border);border-radius:var(--radius-sm);padding:16px;overflow-x:auto;font-size:13px;margin:12px 0}
code{font-family:var(--mono);color:var(--accent)}
.cli-block{
  background:var(--surface);border:2px solid var(--border);
  border-radius:var(--radius-sm);padding:18px;margin:14px 0;font-size:13px;line-height:1.7;
}
.cli-block .comment{color:var(--text-3)}
.cli-block .cmd{color:var(--accent);font-family:var(--mono)}
...
.docs-table th{text-align:left;padding:10px;color:var(--primary-light);font-size:12px;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid var(--border)}
.docs-table td{padding:10px;border-bottom:1px solid var(--surface-3);font-size:13px}
...
.page-pad{{padding-bottom:calc(70px + env(safe-area-inset-bottom))}}
...
.grid-2{{display:grid;gap:14px;margin-bottom:14px}}
@media(min-width:768px){{.grid-2{{grid-template-columns:1fr 1fr}}}}

canvas{{width:100%;height:200px;border-radius:var(--radius-xs)}}
.msg-list{{max-height:240px;overflow-y:auto;font-size:12px}}
.msg-item{{padding:6px 0;border-bottom:1px solid var(--surface-3);color:var(--text-2);word-break:break-all;display:flex;align-items:flex-start;gap:6px}}
...
.quick-chat-box{{display:flex;flex-direction:column;height:200px}}
.quick-chat-msgs{{flex:1;overflow-y:auto;padding:12px;font-size:13px}}
.quick-chat-input{{display:flex;gap:8px;padding:12px;border-top:2px solid var(--border)}}
.quick-chat-input input{{flex:1;background:var(--surface-2);border:2px solid var(--border);color:var(--text);padding:8px 14px;border-radius:var(--radius-xs);font-family:var(--font);font-size:13px;outline:none;transition:var(--transition)}}
.quick-chat-input input:focus{{border-color:var(--primary)}}
.quick-chat-input button{{background:var(--primary);color:#fff;border:2px solid var(--primary-dark);border-radius:var(--radius-xs);padding:8px 16px;cursor:pointer;font-weight:700;font-size:13px;transition:var(--transition);display:flex;align-items:center;gap:4px}}
.quick-chat-input button:hover{{background:var(--primary-dark)}}
.quick-chat-input button svg{{width:16px;height:16px}}

.toast{{position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--surface-2);border:2px solid var(--border);color:var(--text);padding:12px 20px;border-radius:var(--radius-sm);font-size:13px;font-weight:600;box-shadow:var(--shadow-lg);opacity:0;transition:opacity 0.3s;pointer-events:none;z-index:200;max-width:90vw;text-align:center}}
.toast.show{{opacity:1}}
@media(min-width:768px){{.toast{{bottom:24px}}}}

.footer-line{{text-align:center;padding:20px 16px calc(20px + env(safe-area-inset-bottom));color:var(--text-3);font-size:11px}}
</style>
</head>
<body>
...
<div class="topbar">
  ...
  <div class="topbar-right">
    ...
    <button class="nav-item" style="padding:6px 12px;border:2px solid var(--border);border-radius:var(--radius-xs);flex-direction:row;gap:4px;font-size:12px;color:var(--text-2)" onclick="exportCSV()">{i('download',14)} Export</button>
  </div>
</div>
...
<div class="container page-pad">
  ...
  <div class="grid-2">
    ...
    <div class="panel">
      <div class="panel-head">{i('chat',16)} Quick Chat <a href="/chat" style="color:var(--primary-light);font-size:11px;font-weight:600;display:flex;align-items:center;gap:2px">Open Full {i('external',12)}</a></div>
...
async function quickChat(){{const i=$('quick-input');const t=i.value.trim();if(!t)return;i.value='';const box=$('quick-chat');box.innerHTML+=`<div style="color:var(--primary-light);text-align:right;margin:4px 0;font-size:12px">${{esc(t)}}</div>`;try{{const r=await fetch('/api/chat',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{message:t}})}});const d=await r.json();box.innerHTML+=`<div style="color:var(--accent);margin:4px 0;font-size:12px">${{esc(d.reply||'(no reply)')}}</div>`}}catch(x){{box.innerHTML+=`<div style="color:var(--danger);margin:4px 0;font-size:12px">Error</div>`}}box.scrollTop=box.scrollHeight}}
...
<div class="topbar">
  <div class="topbar-logo">{i('brain',26)} DikaAI</div>
  <button class="nav-item" id="sidebarBtn" style="display:none;padding:6px;border:2px solid var(--border);border-radius:var(--radius-xs);flex-direction:row" onclick="toggleSidebar()" aria-label="Menu">{i('menu',20)}</button>
  <nav class="topbar-nav">
  ...
  <div class="topbar-right">
    ...
    <div style="font-size:11px;color:var(--text-3)" id="header-stats">Loading...</div>
  </div>
</div>
  ...
  <main class="chat-main">
    ...
    <div class="chat-input-area">
      <div class="chat-input-wrap">
        ...
        <button class="send-btn" id="sendBtn" onclick="sendFromInput()">{i('send',18)}</button>
      </div>
    </div>
  </main>
...
if(window.innerWidth<=768)document.getElementById('sidebarBtn').style.display='flex';
window.addEventListener('resize',()=>{{document.getElementById('sidebarBtn').style.display=window.innerWidth<=768?'flex':'none'}});
...
<div class="topbar">
  ...
  <div class="topbar-right"><a href="/" style="font-size:12px;color:var(--text-2);display:flex;align-items:center;gap:4px">{i('arrow_right',14)} Back</a></div>
</div>

<div class="docs-body page-pad">
...
<div class="endpoint"><span class="method post">POST</span><span class="path">/v1/chat/completions</span><div class="desc">Chat with DikaAI. OpenAI-compatible format.</div><div class="auth">{i('lock',12)} Required: chat scope</div></div>
...
<div class="endpoint"><span class="method post">POST</span><span class="path">/v1/agent</span><div class="desc">Run coding agent with plan, code, test, debug loop.</div><div class="auth">{i('lock',12)} Required: agent scope</div></div>
...
<div class="endpoint"><span class="method post">POST</span><span class="path">/v1/tools/read</span><div class="desc">Read file content.</div><div class="auth">{i('lock',12)} Required: tools scope</div></div>
<div class="endpoint"><span class="method post">POST</span><span class="path">/v1/tools/search</span><div class="desc">Search codebase.</div><div class="auth">{i('lock',12)} Required: tools scope</div></div>
<div class="endpoint"><span class="method post">POST</span><span class="path">/v1/tools/run</span><div class="desc">Run shell command.</div><div class="auth">{i('lock',12)} Required: tools scope</div></div>
